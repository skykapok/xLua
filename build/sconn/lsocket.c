...
_sock_connect(lua_State *L) {
	...
	if (err != 0) {
		...
		if (err_code == WSAEWOULDBLOCK)
		{
			...
			Sleep(10);
			socklen_t errl = sizeof(err_code);
			getsockopt(sock->fd, SOL_SOCKET, SO_ERROR, (char *) &err_code, &errl);
		}
#endif
		return _push_result(L, err_code);
	}
	else {
...
LUAMOD_API int luaopen_socket_c(lua_State *L) {
    luaL_checkversion(L);
